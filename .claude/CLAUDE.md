...
| **"回填赛果"** | 查赛果 → 回填命中率 → fd 刷收盘价 → 算 CLV → **run.py corpus（语料+就绪度+趋势报告）→ run.py learn（非fd联赛增量拟合+版本发布）** → dc_fit 重拟合 → 复盘报告归档 → git commit | 复盘摘要（方向/比分/CLV/教训+学习就绪度）+ trend.html 胜率趋势（4折线+校准图+方案准确率） |
