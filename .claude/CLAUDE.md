...
- `engine/shadow/`: 影子层（paper.py 影子账本工具+shapes.py 票型引擎+data.py 轮次装载+paper_tickets.json 账本，**纳管 git**；2026-09-07 自 scratch/replay_v2 迁址并重建——旧账本随事故丢失从空账重启，qcache 快照生成器 freq_snap 未重建（ttg/crs 影子票跳过中），回放编排 run_replay/baseline/stats 不重建（结论已沉淀 04-summaries））
