...
| 用户说 | 系统自动走完 | 产出 |
|:---|:---|:---|
| **"帮我预测"** | run.py update（刷数据+联赛画像）→ **data_check 体检+冷启动初始化（缺联赛/球队画像先补）** → dc_fit --auto → 体彩采集 → 本地检索+ESPN 实时积分榜 → DC 融合 → 战意状态机 → 分析评级 → 报告+预测JSON 归档 → **影子层登记：`python engine/shadow/paper.py shadow-all <销售日>`（全方案影子票冻结赛前五池真实价，阶段2 complex-test，幂等；2026-09-07 迁址 engine/shadow 纳管 git+shapes.py 按设计规格重建——19 spec 中 ttg/crs 6 个依赖 qcache 快照（freq_snap 生成器未重建）自动跳过；**日期用体彩销售日：当晚开赛场挂次日凌晨销售日，如周一晚轮=2026-09-08**）** → git commit | 摘要卡（方案/赔率/仓位） |
| **"再跑一遍"** | 体彩赔率复扫 → 信息边际三定律判定 → 增量更新报告 → git commit | 终审结论（维持/修订） |
| **"回填赛果"** | 查赛果 → 回填命中率 → fd 刷收盘价 → 算 CLV → **票务结算（backfill.settle_tickets：pending票对赛果→按形状算派彩→重刷 tickets.html）** → **run.py corpus（语料+就绪度+趋势报告）→ run.py attribute（错题归因→attribution.json）→ run.py learn（非fd联赛增量拟合+版本发布）** → dc_fit 重拟合。回填时自动增补 `pinClose/pinSource`（fd Pinnacle 收盘三键匹配·日±1+比分+联赛→去水三向，ambiguous 诚实降级；F3/F4 市场锚判别地基），v4.7 起新轮 matches[] 带 `lambdaHome/lambdaAway/fusedPre/chainSteps`（归因 F1/F5 精确判别） → **影子层结算：`python engine/shadow/paper.py settle-all <销售日>`（影子票对真实赛果结算+新比赛日 shadow-all 登记，阶段2 时间外数据积累；日期口径同上=体彩销售日）** → 复盘报告归档 → git commit | 复盘摘要（方向/比分/CLV/教训+学习就绪度）+ tickets.html 实票账本（资金曲线/玩法分解）+ trend.html 胜率趋势（4折线+校准图+方案准确率）+ attribution.json（偏差归因账本：F5/F9/F10 因子分布） |
...
- `engine/scripts/scratch/`: 临时分析脚本目录（已 gitignore 不进 git；会话结束清理；PowerShell 下**禁止 python -c 内联多行脚本**——转义坑 09-04 一日三次实证，一律写 .py 脚本文件执行；**凭证/持久数据禁放此目录——09-07 影子层 shapes.py+账本被清理误删事故教训**）
- `engine/shadow/`: 影子层（paper.py 影子账本工具+shapes.py 票型引擎+data.py 轮次装载+paper_tickets.json 账本+freq_snap.py qcache 快照生成器（2026-09-08 重建：与 boldplay 主链同源 λ 平移链产 crs/ttg 候选→C/D 族 6 spec 影子票恢复登记；crs 池键 sXXsYY 换算 'h:a'；冻结纪律=文件在即返回 --force 才重写）+qcache/ 日期快照（赛前判断凭证链，纳管 git），**纳管 git**；2026-09-07 自 scratch/replay_v2 迁址并重建——旧账本随事故丢失从空账重启，回放编排 run_replay/baseline/stats 不重建（结论已沉淀 04-summaries））
...
4. 融合：p_final = σ(0.4·logit(p_DC) + 1.0·logit(p_pinnacle))，系数存 engine/cache/fusion.json；**leagueOverrides 联赛级 a 分层（2026-09-15 荷甲立项+大哥拍板德甲意甲同切）**：荷甲/德甲/意甲 a=0（2526 walk-forward DC 方向单调负贡献，a=0 纯市场锚改善 12.6%/5.4%/4.2% 均超 1% 护栏）——DC 负贡献联赛方向全听市场、比分玩法仍用 DC 矩阵；统一读 common.load_fusion_ab(league)，dc_predict/boldplay/backtest 三链生效
