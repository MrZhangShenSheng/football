...
> *实测 2026-08-22（64 场均值）。核心纪律：**同一注串内不同玩法不可混**（官方规则第七条）；同场次跨票分池允许（v4.9 拍板口径）；抽水率=期望亏损率，串N关期望=(1-抽水)^N——高抽水池玩法串越长亏越快。
...
│   └── 05-trends/         #    趋势发现
├── engine/                ← ② 计算层
│   ├── scripts/           #    run.py(入口) / dc_fit / dc_predict / backtest / corpus / trend_report / backfill / calibrate / ablate / odds_fetch / elo_fetch / xg_fetch / espn_fetch / cn_fetch / sporttery_fetch / band_calibration / score_ev / live_odds_probe / boldplay(阶梯出票卡) / build_index
